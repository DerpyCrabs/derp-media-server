...
test.describe('Tiling Layout Picker', () => {
  ...
  test('tiling picker hover highlight moves from first quarter cell to second', async () => {
    ...
    const previewZ = Number(await workspacePreview.evaluate((el) => getComputedStyle(el).zIndex))
    const pickerZ = Number(await picker.evaluate((el) => getComputedStyle(el).zIndex))
    expect(previewZ).toBeLessThan(pickerZ)
