...
function extractSnippet(content: string, query: string): string {
  ...
  // Context too long: show only the matching line so the match is never cut off
  const matchPos = matchLine.toLowerCase().indexOf(lowerQuery)
  if (matchLine.length <= SNIPPET_MAX) return matchLine
  // Match line is long: center snippet around the match
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!knowledgeBases.includes(root.replace(/\\/g, '/'))) {
      return NextResponse.json({ error: 'Not a knowledge base' }, { status: 400 })
    }
