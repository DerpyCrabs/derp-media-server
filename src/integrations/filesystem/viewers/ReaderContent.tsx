...
import { ReaderSelectionMenu, type ReaderSelection } from '../../../reader/ReaderSelectionMenu'
import { menuPositionForRect, visibleRectForRange } from '../../../reader/reader-geometry'
import { buildMediaUrl } from '@/lib/api-media-urls'
import { parseBook } from '../../../reader/book-parser'
import { renderBook, type RenderedBook } from '../../../reader/book-sanitize'
import { BookContent } from '../../../reader/BookContent'
import { ReaderOutline, type ReaderOutlineItem } from '../../../reader/ReaderOutline'
import {
...
} from '../../../reader/reader-state-client'
...
type ReaderPersistedSnapshot = {
  activePath: string
  next: ReaderSyncedState
  revision: number
  fingerprint: string
}
...
const naturalCompare = (left: ResourceSummary, right: ResourceSummary) =>
...
export function ReaderContent(props: ReaderContentProps) {
  ...
  let previousDocument: { path: string; kind: ReaderContentProps['sourceKind'] } | null = null
  let capturePreviousState: (
    activePath: string,
    activeKind: ReaderContentProps['sourceKind'],
  ) => ReaderPersistedSnapshot | null = () => null
  let persistPreviousState: (snapshot: ReaderPersistedSnapshot) => Promise<void> = () =>
    Promise.resolve()
  const path = createMemo(() => props.sourcePath)
  const sourceKind = createMemo(() => props.sourceKind)
  ...
  const fullscreenRoot = () => readerRoot.closest<HTMLElement>('[data-reader-shell]') ?? readerRoot
  ...
  createEffect(() => {
    ...
    if (previous && (previous.path !== activePath || previous.kind !== kind)) {
      window.clearTimeout(saveTimer)
      const snapshot = untrack(() => capturePreviousState(previous.path, previous.kind))
      if (snapshot) void persistPreviousState(snapshot)
    }
    previousDocument = { path: activePath, kind }
    ...
    void Promise.all([
      ...
      .then(async ([saved, preferenceEnvelope]) => {
        ...
        if (kind === 'folder') {
          const payload = await listFilesystemResources({ dir: activePath })
          const resources = payload.resources
            ...
            .sort(naturalCompare)
          if (resources.length === 0) throw new Error('Folder contains no supported images')
          const loaded = await Promise.all(
            resources.map(async (resource) => {
              const path = filesystemPathForResourceKey(resource.key)
              if (path === null) throw new Error('Folder contains an invalid filesystem resource')
  ...
  const capturePersistedState = (
    activePath = path(),
    activeKind: ReaderContentProps['sourceKind'] = sourceKind(),
  ): ReaderPersistedSnapshot | null => {
    ...
    const next: ReaderSyncedState = {
      ...
      selectionMode: preferredSelectionMode(),
      defaultAction: defaultAction(),
      chapterId: activeKind === 'book' ? currentChapterId() : undefined,
      anchor: activeKind === 'book' ? currentAnchor : undefined,
      progress: activeKind === 'book' ? bookProgress() : undefined,
      chapterProgress: activeKind === 'book' ? liveChapterProgress : undefined,
  ...
  capturePreviousState = capturePersistedState

  const persistNow = async (snapshot: ReaderPersistedSnapshot) => {
    const { activePath, next, revision, fingerprint } = snapshot
    const stillCurrent = () => path() === activePath
    const saved = await saveSyncedReaderState(
      activePath,
      next,
      stillCurrent() && stateRevision() !== revision ? stateRevision() : revision,
      stillCurrent() && stateFingerprint() !== fingerprint ? stateFingerprint() : fingerprint,
    ).catch(() => null)
    if (!saved) {
  ...
  const enqueuePersistedState = (snapshot: ReaderPersistedSnapshot) => {
  ...
  persistPreviousState = enqueuePersistedState

  const persist = () => {
    const snapshot = capturePersistedState()
    return snapshot ? enqueuePersistedState(snapshot) : Promise.resolve()
  }
  ...
  onMount(() => {
    ...
    if (!activeReaderRoot) activeReaderRoot = readerRoot
  ...
  createEffect(() => {
    ...
    const keydown = (event: KeyboardEvent) => {
      ...
      const targetReader = target?.closest<HTMLElement>('[data-reader-root]')
      const ownsEvent = targetReader
        ? targetReader === readerRoot
        : document.fullscreenElement === fullscreenRoot() || activeReaderRoot === readerRoot
      if (!ownsEvent) return
      if (event.key === 'Escape') {
        ...
        else if (props.closeOnEscape && props.onClose) void close()
        return
      }
  ...
  return (
    <Show when={path()}>
      <div
        ...
        class='relative flex h-full min-h-0 flex-col bg-neutral-900 text-white'
        data-reader-root
