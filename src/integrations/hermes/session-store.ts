...
registerHermesSessionLiveStatus((sessionId) => {
...
async function refreshHermesCapabilities(key: string) {
  try {
    const payload = await jsonRequest(hermesTransportRoutes.capabilities)
...
export async function transcribeHermesAudio(key: string, blob: Blob) {
  ...
  const payload = await jsonRequest(hermesTransportRoutes.transcribe, {
    method: 'POST',
...
export async function speakHermesText(text: string) {
  replyAudio?.pause()
  const payload = await jsonRequest(hermesTransportRoutes.speak, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function refreshHermesChat(key: string) {
  ...
  try {
    ...
    const [payload, detail] = await Promise.all([
      jsonRequest(hermesSessionMessagesUrl(state.sessionId, historyLimit, 0)),
      jsonRequest(hermesSessionUrl(state.sessionId)),
    ])
...
export async function loadOlderHermesMessages(key: string) {
  ...
  try {
    const payload = await jsonRequest(
      hermesSessionMessagesUrl(state.sessionId, 100, state.messages.length),
    )
...
async function refreshHermesModelOptions(key: string) {
  try {
    const payload = await jsonRequest(hermesTransportRoutes.modelOptions)
    if (!sessions[key]) return
...
function connectEvents() {
  ...
  eventSource = new EventSource(hermesTransportRoutes.events)
  eventSource.onopen = () => {
    for (const key of Object.keys(sessions))
...
async function refreshHermesCompletions(key: string, value: string) {
  ...
  try {
    ...
    const payload = await jsonRequest(hermesCompletionsUrl(params))
...
export async function renameHermesSession(key: string, title: string) {
  ...
  if (!sessionId) return
  await jsonRequest(hermesTransportRoutes.rename, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function branchHermesSession(key: string, name?: string, count?: number) {
  ...
  const payload = await jsonRequest(hermesTransportRoutes.branch, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function rewindHermesSession(key: string, messageId: string, replacement?: string) {
  ...
  try {
    await jsonRequest(hermesTransportRoutes.rewind, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function exportHermesSession(key: string) {
  ...
  const outcome = await runIntegrationAction(
    hermesResourceKey('session', state.sessionId),
    'hermes.download',
  )
  const data =
    outcome.data && typeof outcome.data === 'object' && !Array.isArray(outcome.data)
      ...
      : null
  if (typeof data?.url !== 'string') throw new Error('Hermes export action returned no URL')
  const content = JSON.stringify(await jsonRequest(data.url), null, 2)
...
export async function steerHermesPrompt(key: string) {
  ...
  await jsonRequest(hermesTransportRoutes.steer, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function sendHermesPrompt(key: string, takeover = false): Promise<string | undefined> {
  ...
  try {
    const payload = await jsonRequest(hermesTransportRoutes.turn, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
export async function stopHermesTurn(key: string) {
  const sessionId = sessions[key]?.sessionId
  if (!sessionId) return
  await jsonRequest(hermesTransportRoutes.stop, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function restoreHermesSession(key: string) {
  const sessionId = sessions[key]?.sessionId
  if (!sessionId) return
  await jsonRequest(hermesTransportRoutes.restore, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ sessionId }),
  })
...
export async function archiveHermesSession(key: string) {
  const state = sessions[key]
  if (!state?.sessionId) return
  await jsonRequest(hermesTransportRoutes.archive, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function answerHermesDecision(key: string, answer: string) {
  ...
  try {
    await jsonRequest(hermesTransportRoutes.decision, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
