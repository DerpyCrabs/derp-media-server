...
export function HermesChatPane(props: HermesChatPaneProps) {
  const key = createMemo(() => ensureHermesChat(props.content()))
  const state = () => hermesSessions[key()]
  const owner = () => props.instanceId() || key()
  ...
  createEffect(() => {
    ...
    if (sessionId && sessionId !== props.content().sessionId) props.onSessionCreated?.(sessionId)
  })
  createEffect(() => {
  ...
  createEffect(() => {
    ...
    if (title && title !== props.title?.()) props.onTitleChanged?.(title)
  })
