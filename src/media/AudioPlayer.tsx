...
function sameQueue(left: readonly PlaybackItem[], right: readonly PlaybackItem[]) {
  return (
...
export function AudioPlayer() {
  ...
  createEffect(() => {
    ...
    if (!item || !shouldHandleAudio() || !('mediaSession' in navigator)) return
    const metadata = audioMetadata()
    const artworkUrl = displayImageUrl()
    navigator.mediaSession.metadata = new MediaMetadata({
      title: isVideoFile() ? `${item.name} (Audio)` : metadata?.title || item.name,
      artist: isVideoFile() ? 'Video Audio' : metadata?.artist || 'Unknown Artist',
      album: metadata?.album || currentDir() || 'Unknown Album',
      ...(artworkUrl
        ? {
            artwork: [
              {
                src: artworkUrl.startsWith('data:')
                  ? artworkUrl
