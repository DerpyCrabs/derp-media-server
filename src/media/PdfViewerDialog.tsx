  ...
  return (
    <Show when={viewingPath() && isPdf()}>
      <div
        ...
        class='fixed inset-0 z-50 flex flex-col bg-neutral-900'
      >
        <div class='flex min-h-14 flex-wrap items-center gap-1 bg-black/70 px-2 py-1 text-white'>
          ...
          <button
            ...
            class='inline-flex h-11 w-11 items-center justify-center rounded-md hover:bg-white/10'
            disabled={page() <= 1}
          ...
          <span class='hidden min-w-12 text-center text-sm min-[420px]:inline'>
        ...
        <div class='flex flex-1 items-start justify-center overflow-auto p-3'>
          <Show
            when={!error()}
            fallback={
              <p role='alert' class='m-auto text-white'>
                {error()}
              </p>
            }
          >
