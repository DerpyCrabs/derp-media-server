...
export function WorkspacePage() {
  ...
  const { onDragMove, onDragEnd } = useSnapZones()
    ...
    () =>
      windows
        ...
        .map((window) => {
          ...
          return {
            id: window.id,
            label: tabCount > 1 ? `${window.title} (+${tabCount - 1})` : window.title,
            ...
            icon: getIcon(
              ...
              (window.iconType ??
                (window.type === 'player' ? MediaType.VIDEO : MediaType.OTHER)) === MediaType.VIDEO,
