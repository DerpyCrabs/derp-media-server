...
export function OfflineStatus() {
  ...
  onMount(() => {
    ...
    const listener = (event: Event) => {
      ...
      setJobs((current) =>
        [detail, ...current.filter((job) => job.path !== detail.path)].slice(0, 20),
      )
    ...
    <>
      <Show when={jobs()[0]} keyed>
        {(job) => (
          <button
            type='button'
            class='fixed right-3 bottom-[calc(0.75rem+env(safe-area-inset-bottom,0px))] z-10020 min-h-11 max-w-[calc(100vw-1.5rem)] rounded-lg border border-border bg-popover px-4 py-3 text-left text-sm shadow-lg'
            onClick={() => setManagerOpen(true)}
          >
            <span class='block font-medium'>{message(job)}</span>
                  ...
                  <article class='rounded-lg border p-3'>
                    <div class='flex items-center justify-between gap-2'>
                      <div class='min-w-0'>
                        ...
                        <p class='text-muted-foreground text-xs'>{message(job)}</p>
                      </div>
                      <div class='flex shrink-0 gap-1'>
                        <Show when={job.state === 'queued' || job.state === 'running'}>
                          <button
                            class='min-h-11 rounded-md px-3'
                            onClick={() => job.path && cancelWebOffline(job.path)}
                          >
                            Cancel
                          </button>
                        </Show>
                        <Show when={job.state === 'failed' || job.state === 'cancelled'}>
                          <button
                            class='min-h-11 rounded-md px-3'
                            onClick={() => job.path && retryWebOffline(job.path)}
                          >
                            Retry
                          </button>
                        </Show>
