...
export type ResourceViewerContentProps = {
  runtime: ContentRuntime
  contentInstance: Accessor<ResourceContentInstance | null>
  contentVisible: Accessor<boolean>
  viewingPath: Accessor<string>
  readerKind?: Accessor<'pdf' | 'folder' | 'book' | null>
  directory?: Accessor<string>
  active?: Accessor<boolean>
  onReplaceContent?: (instance: ContentInstance) => void
...
export function ResourceViewerContent(props: ResourceViewerContentProps) {
  ...
  const readerSourceKind = createMemo<'pdf' | 'folder' | 'book' | null>(() => {
    if (readerKind()) return readerKind()
    if (mediaType() === MediaType.PDF) return 'pdf'
    if (mediaType() === MediaType.BOOK) return 'book'
    return null
  })
  const mountableContent = createMemo(() => {
    const type = mediaType()
    return type === MediaType.AUDIO || type === MediaType.VIDEO ? null : props.contentInstance()
  })

  function updateViewing(path: string) {
    props.onNavigateViewing(path)
  }
      ...
      <div class='min-w-0'>
        ...
        <Show when={!props.compact}>
          <p class='truncate text-[11px] text-muted-foreground/75'>
            {audioMetadataQuery.data?.album || directory() || 'Unknown album'}
          </p>
  ...
  return (
    ...
    >
      <Show when={mountableContent()}>
        <ContentRuntimeView
          runtime={props.runtime}
          instance={mountableContent}
          active={contentActive}
          onReplace={props.onReplaceContent}
          onClose={props.onClose}
        />
      </Show>

      <Show when={!readerSourceKind() && mediaType() === MediaType.VIDEO && viewingPath()}>
      ...
      <Show when={!readerSourceKind() && mediaType() === MediaType.AUDIO && viewingPath()}>
        <div
