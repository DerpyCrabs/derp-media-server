...
function validItem(value: unknown): value is PlaybackItem {
  ...
  return !!(
    item.resource &&
    typeof item.resource.provider === 'string' &&
...
export function playbackItemKey(value: Pick<PlaybackItem, 'resource'>): string {
  const { provider, id } = value.resource
  return `${provider.length}:${provider}${id.length}:${id}`
}
