...
#[cfg(test)]
mod tests {
    use super::super::config::HermesFilesystemMode;
    use super::*;
    use std::sync::{
        Arc,
    ...
    fn opaque_session_ids_cannot_change_gateway_route() {
        for valid in [
            " ".to_string(),
            "opaque?query#fragment".to_string(),
            "a".repeat(512),
            "é".repeat(256),
            "😀".repeat(128),
        ] {
            assert_eq!(validate_opaque_id(&valid).unwrap(), valid);
        }
        ...
        for invalid in [
            ...
            "a%2fb".to_string(),
            "a\nb".to_string(),
            format!("a{}b", char::from_u32(0x85).unwrap()),
            "a".repeat(513),
            "é".repeat(257),
            "😀".repeat(129),
        ] {
            assert!(
                validate_opaque_id(&invalid).is_err(),
                "accepted {invalid:?}"
            );
        }
