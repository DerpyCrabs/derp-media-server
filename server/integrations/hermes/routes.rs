...
pub(crate) fn session_export_url(id: &str) -> AppResult<String> {
...
fn valid_event_id(value: Option<&str>) -> Option<String> {
    value
        .filter(|id| validate_opaque_id(id).is_ok())
        .map(str::to_string)
}
...
async fn publish_hermes_event(runtime: &HermesRuntime, value: Value) {
    let value = normalize_hermes_event(&runtime.runtime_ids, &runtime.active_ids, value).await;
    let _ = runtime.events.send(value);
}

pub(crate) fn start_event_bridge(
    runtime: &std::sync::Arc<HermesRuntime>,
    mut receiver: tokio::sync::broadcast::Receiver<Value>,
) {
    let runtime = std::sync::Arc::downgrade(runtime);
        ...
        loop {
            match receiver.recv().await {
                Ok(value) => {
                    ...
                    publish_hermes_event(&runtime, value).await;
                }
                Err(tokio::sync::broadcast::error::RecvError::Lagged(_)) => {
                    if let Some(runtime) = runtime.upgrade() {
                        runtime.runtime_ids.lock().await.clear();
                        runtime.active_ids.lock().await.clear();
                    } else {
                        break;
                    }
                }
...
async fn turn(State(state): State<Shared>, Json(body): Json<TurnBody>) -> AppResult<Json<Value>> {
    ...
    let (runtime_id, stored_id) = if let Some(stored_id) = body.session_id {
        if let Some(runtime) = hermes.runtime_ids.lock().await.get(&stored_id).cloned() {
            (runtime, stored_id)
        } else {
            let resumed = hub
                .rpc(
    ...
    hermes.active_ids.lock().await.insert(stored_id.clone());
    let gateway = hermes.transport.clone();
    let durable_for_event = stored_id.clone();
    let runtime_ids = hermes.clone();
...
mod tests {
    ...
    #[test]
    fn export_url_uses_owned_route_and_encodes_opaque_id() {
        assert_eq!(
            session_export_url("opaque?query#fragment").unwrap(),
            "/api/hermes/sessions/opaque%3Fquery%23fragment/export"
        );
        assert!(session_export_url("../escape").is_err());
    }
...
pub fn router() -> Router<Shared> {
    Router::new()
        ...
        .route(SESSION_EXPORT_ROUTE, get(export_session))
